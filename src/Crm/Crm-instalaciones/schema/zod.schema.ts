...
export const crearInstalacionSchema = z
  ...
  .superRefine((values, ctx) => {
    ...
    if (requierePrealtaPppoe && values.servicioInternetId === null) {
      ctx.addIssue({
        code: "custom",
        path: ["servicioInternetId"],
        message: "Seleccione el plan que se homologará para la cuenta PPPoE",
      });
    }

    if (requierePrealtaPppoe && values.acceso.mikrotikRouterId === null) {
      ctx.addIssue({
        code: "custom",
        path: ["acceso", "mikrotikRouterId"],
        message: "Seleccione el router donde se preparará la cuenta PPPoE",
      });
    }
