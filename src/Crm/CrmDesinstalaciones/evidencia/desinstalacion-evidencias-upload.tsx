...
import { usePostEvidenciaDesinstalacion } from "@/Crm/CrmHooks/hooks/desinstalaciones/desinstalaciones-hook";

import ImagesCropper from "@/Crm/Helpers/CutterImages/ImageCropper";

import {
  buildEvidenciaDesinstalacionFormData,
  type EvidenciaDesinstalacionDraft,
  type EvidenciaDesinstalacionUploadStatus,
} from "@/Crm/features/desinstalaciones/evidencias-payload";
...
const TIPO_OPTIONS = Object.values(TipoEvidenciaClienteOperacion).map(
  (value) => ({
    value,
...
function getStatusData(status: EvidenciaDesinstalacionUploadStatus): {
  label: string;
...
type EvidenciaCardProps = {
  evidencia: EvidenciaDesinstalacionDraft;

  disabled?: boolean;

  onChange: (id: string, patch: Partial<EvidenciaDesinstalacionDraft>) => void;

  onRemove: (id: string) => void;
};

const EvidenciaDesinstalacionCard = memo(function EvidenciaDesinstalacionCard({
  evidencia,
...
export const DesinstalacionEvidenciasUpload = memo(
  function DesinstalacionEvidenciasUpload({
    desinstalacionId,
    ...
    const uploadMutation = usePostEvidenciaDesinstalacion(desinstalacionId);

    const fileInputRef = useRef<HTMLInputElement>(null);

    const evidenciasRef = useRef<EvidenciaDesinstalacionDraft[]>([]);

    const [cropCandidates, setCropCandidates] = useState<File[]>([]);

    const [evidencias, setEvidencias] = useState<
      EvidenciaDesinstalacionDraft[]
    >([]);
    ...
    const appendFiles = useCallback((files: File[]) => {
      setEvidencias((current) => {
        const nuevas = files.map(
          (file, index): EvidenciaDesinstalacionDraft => ({
    ...
    const handleSelectFiles = useCallback(
      ...
      [appendFiles, cropDialog, cropImages],
    );
    ...
    const handleUpdateEvidencia = useCallback(
      (
        id: string,

        patch: Partial<EvidenciaDesinstalacionDraft>,
      ) => {
        setEvidencias((current) =>
    ...
    const handleRemoveEvidencia = useCallback((id: string) => {
      setEvidencias((current) => {
    ...
    const handleClearEvidencias = useCallback(() => {
      setEvidencias((current) => {
    ...
    const handleSubmit = useCallback(async () => {
      ...
      toast.warning(
        `Se subieron ${uploadedIds.size} de ${evidencias.length} evidencias. Las imágenes fallidas permanecen para reintentar.`,
      );
                ...
                <AppInline align="center" gap="xs">
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate text-xs font-semibold">
                      Evidencias de desinstalación
                    </h2>

                    <p className="text-[10px] text-[hsl(var(--app-muted-foreground,var(--muted-foreground)))]">
                      Cada imagen puede tener su propio tipo, descripción y
                      orden.
                    </p>
                  </div>
                </AppInline>
            ...
            {evidencias.length === 0 ? (
              ...
              <AppStack gap="xs">
                {evidencias.map((evidencia) => (
                  <EvidenciaDesinstalacionCard
                    key={evidencia.id}
                    evidencia={evidencia}
                    disabled={disabled || isSubmittingBatch}
                    onChange={handleUpdateEvidencia}
